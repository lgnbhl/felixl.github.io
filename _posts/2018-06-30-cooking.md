...
excerpt: Building classification models to predict the dish's cuisine from its ingredients.
permalink: /cooking/
...
1.  Reproducing yilisg’s R analysis in a tidy way, using the tidyverse
    ecosystem (and adding so code of my own).
...
The accuracy score of our Stacked Ensemble model is 0.79314. It is better than our previous xgboost model (0.7871). However, it is hard to judge in terms of algorithm performance, as h2o's AutoML ran twice the time of xgboost on my computer. 
