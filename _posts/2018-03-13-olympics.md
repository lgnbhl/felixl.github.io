...
excerpt: Using regression models to explore the relationship between the HDI and the medals won during the Winter Olympics.
permalink: /olympics/
...
Like most of us, I watched the Olympics Winter Games. But after seeing the [medal table](https://www.pyeongchang2018.com/en/game-time/results/OWG2018/en/general/medal-standings.htm), I had the impression that the richer the country was, the more medals he got. But was it really the case? And if yes, to what extent? These are questions we can answer using R.

But what is a rich country? We need to choose a metric to classify the participating nations from the richer to the poorer. As Norway, Germany and Canada are the most medaled countries of the 2018 Winter Olympics, the Human Development Index is clearly a better choice than the GDP.
...
-   Is the Human Development Index a good predictor to know if a country will win or not more than a certain amount of medals? (binary logistic regression)

Our objective is not to make sport prediction, but simply to know how much the Human Development Index can explain the number of medals won by each country during the Winter Olympics Games. This article also wants to show how to use simple models (linear and logistic regressions) for exploratory analysis.
...
If you are not interested in the scraping part, just go to the next chapter.
...
Let's begin by making a simple linear regression with the variables `nMedals` and `HDI` for the 2018 Winter Olympics Games.
...
Our model have a coefficient of determination (or R-squared) of `0.21`, with a p-value of `0.01`. The R-squared means that 21% of the variation of the `nMedals` can be explained by the variation of the `HDI`. It is quite low but not that bad given the fact that we are using only one predictor.

Looking at the plot can teach us two things. Firstly, the countries having won more than three medals have all a HDI higher than 0.86. Secondly, being a big and ambitious country (like China or Russia) or not having a strong ski culture (like Australia) could potentially be good predictors.
...
Our previous model didn't take into account the countries which haven't won any medals. This is a biais we need to study now.
...
We will now make a logistic regression. In other words, we will model the probability that a country has won more than 3 medals during the 2018 Winter Olympics Games according to its HDI.
...
In our visualization, we can recognize once again the two countries, i.e. China and Russia, which are classified as `1` (won more than 3 medals) while having a HDI higher than 0.87. Our binary classification reveals that some countries with a HDI higher than 0.87 have won less than 4 medals. Which are they?
...
These twelve countries call for other predictors in order to improve our models. But this goes beyond our initial questions.

## Conclusion

Our first model (a simple linear regression) showed that China and Russia are the only two countries with a Human Development Index (HDI) smaller than 0.87 which have won more than 3 medals during the 2018 Winter Olympics Game. We built then linear regressions for each Winter Olympics Games since 1992 and saw that the HDI is increasingly significant. Lastly, our logistic regression revealed that twelve countries with a HDI higher than 0.87 have won less than 4 medals during the 2018 Winter Olympics.
