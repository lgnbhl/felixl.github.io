...
Let's use cross-validation to confirm our observation. 

We will build nested data frame following the [chapter 5](http://r4ds.had.co.nz/many-models.html) of Garrett Grolemund and Hadley Wickham book *R for Data Science* and with the help of [this](http://jacobsimmering.com/2016/11/11/CrossValidationInR/) excellent article.

```{r}
library(modelr)

getModels_cv <- function(i, j) {
  ...
  data_models <- data_cv %>% 
    ...
    mutate(model = map(train, ~ lm(nMedals ~ HDI, data = .))) %>%
...
set.seed(1)
models_cv <- map2_dfr(wo_years, hdi_years, getModels_cv)
```

Now that we have our nested dataframe, let's unnest it!

Using the 10-fold cross-validation of our eigth models (1992 to 2018), we will build boxplots to visualize the variability of the p-value and the r-squared.

```{r}
tidyr::unnest(models_cv, metrics) %>%
  ggplot(aes(x = as.factor(year), y = r.squared)) +
  geom_boxplot() +
  ggthemes::theme_economist_white() +
  theme(plot.background = element_rect(fill = "#f8f2e4")) +
  labs(x = NULL, y = "R-squared",
       title = "Model Cross-validation",
       subtitle = "Evolution of the r-squared, k=10",
       caption = "Félix Luginbühl (@lgnbhl)\nData source: Wikipedia")


tidyr::unnest(models_cv, metrics) %>%
  ggplot(aes(x = as.factor(year), y = p.value)) +
  geom_boxplot() +
  ggthemes::theme_economist_white() +
  theme(plot.background = element_rect(fill = "#f8f2e4")) +
  labs(x = NULL, y = "P-value",
       title = "Model cross validation, k=10",
       subtitle = "Evolution of the p-value",
       caption = "Félix Luginbühl (@lgnbhl)\nData source: Wikipedia")
```

![](/images/chart_olympics_cross1.png)
![](/images/chart_olympics_cross2.png)


Yes, cross-validations seem to confirm our initial observation. The R-squared is increasing over the year while the p-value is decreasing.
...
The model without China and Russia explains one third (33%) of the variability and is statistically significant (p-value < 0.005).
