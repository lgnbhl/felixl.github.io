...
Let’s beginn by visualizing the relationship between the variables
...
We can observe that China and Russia are not well explained by a linear
regression.

However, in order to complete all the assumptions required to run a
...
We only visualized the year 2018. What is the relation between our two
...
Linear regressions show that the Human Development Index is increasingly
