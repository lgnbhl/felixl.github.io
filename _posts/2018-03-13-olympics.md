...
data_medals <- dataset %>%
  ...
  magrittr::set_colnames(c("Country", "Medal", "Number")) %>%
  filter(!(Number == 0)) %>%
  arrange()

set.seed(20)
data_medals %>%
  ggplot(aes(x = fct_reorder(Country, Number), y = Number)) +
...
Let’s beginn with a simple linear regression with the variables `nMedals` and `HDI` for the 2018 Winter Olympics Game.
