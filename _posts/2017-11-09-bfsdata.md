...
We got only four datasets related to the occurrence "student" in the English langage. Obviously, the Swiss Federal Statistical Office has much more data in German and French. As explained before, the package creates a dataset in the Global Environment called `bfs_metadataSubset` for each search in the metadata download from the BFS official website. Be warned that `bfs_metadataSubset` is replaced every time `bfs_search` is called.
