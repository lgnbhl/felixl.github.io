...
Note that this could also have been done on the BFS online database, with the same result:
...
In 1999, Switzerland adhered to the "Bologna Process", which harmonized academic qualifications in three levels: Bachelor, Master and Doctorate. In the graphics, we can see its progressive implementation by the Swiss universities since the 2000s.
...
Below are the three function of the {bfsdata} package.
