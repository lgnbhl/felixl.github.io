...
Note that you can have access to the metadata of your BFS dataset by reading directly the PX file in R with the {[pxR](https://cran.r-project.org/web/packages/pxR/index.html)} package, also saved in your Global Environnement while using the `bfs_download` function. Here are three examples of metadata taken from the ```bfsData_student_px``` object.
