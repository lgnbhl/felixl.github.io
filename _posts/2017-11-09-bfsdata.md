...
When using `bfs_search`, you will have in your Global Environment a CSV file called `bfsMetadata`, which is the Excel file from the [BFS website](https://www.pxweb.bfs.admin.ch/default.aspx?px_language=en) accessible through the "list of the cubes" link. You will also get a CSV dataset which contains the result of your search, named `bfsMetadataSubset`.
...
Note that you can have access to the metadata of your BFS dataset by reading directly the PX file in R with the {[pxR](https://github.com/cjgb/pxR)} package, also saved in your Global Environnement while using the `bfs_download` function. Here are three examples of metadata taken from the ```bfsData_student_px``` object.
