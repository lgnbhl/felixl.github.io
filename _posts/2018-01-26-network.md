---
layout: single
title: Which Marvel Superhero is the most central?
excerpt: A social network analysis of the Marvel cinematic universe with {tidygraph}, {ggraph} and {visNetwork}.
permalink: /network/
date: 2018-01-26
author: lgnbhl
...
ggraph(marvel_graph, layout = "nicely") + 
  ...
  geom_node_point(aes(size = degree, color = as.factor(type))) + 
...
marvel_graph %>%
  ...
  geom_edge_diagonal(alpha = 0.2) + 
  geom_node_point(aes(color = as.factor(group_walktrap))) + 
  geom_node_text(aes(label = name), size = 2.5, repel = TRUE) +
